...
GEMINI_API_KEY = "AIzaSyDqAYD6aPGt9FNGI55rBFQDqPdcjwTNnCg"  # genai.configure(api_key=GEMINI_API_KEY)
...
#Function to process Excel to JSON
...
if uploaded_files:
    ...
    if selected_pdf:
        ...
        NOTES_EXCEL_PATH = f"temp/{base_name}_AllNotes.xlsx"
        NOTE_EMBEDDINGS_CACHE = f"temp/{base_name}_note_embeddings.json"
        ...
        # Process question for splitting
        ...
        if not os.path.exists(JSON_FILE_STREAM) or not os.path.exists(JSON_FILE_LATTICE):
            ...
            with pd.ExcelWriter(EXCEL_FILE_STREAM, engine='xlsxwriter') as writer:
                ...
                for i, table in enumerate(stream_tables):
                    ...
                    new_table.to_excel(writer, sheet_name=f"Stream_Table_{table_index}", index=False, header=False)
            ...
            with pd.ExcelWriter(EXCEL_FILE_LATTICE, engine='xlsxwriter') as writer:
                ...
                for i, table in enumerate(lattice_tables):
                    ...
                    # Combine metadata and table
                    new_table = pd.concat([metadata, table_df], ignore_index=True)
                    new_table.to_excel(writer, sheet_name=f"Lattice_Table_{table_index}", index=False, header=False)
        ...
        st.success("Notes extraction and embedding complete. Excel and embedding files saved.")
        ...
        if query:
            ...
            for q in query1:
                try:
                    ...
                    # st.write(f"retrieved_table_rows_stream: {retrieved_table_rows_stream}")
                    # st.write(f"retrieved_table_rows_lattice: {retrieved_table_rows_lattice}")
                    # st.write(f"retrieved_text_chunks: {retrieved_text_chunks}")
